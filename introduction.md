# Introduction

The document is aimed to assist third parties to integrate with the new API Service which is not only third party-agnostic but also easily integrable as a standalone service or in combination with other Docs offerings. The Global Docs Service now has a new Global Docs API Gateway which decouples Docs features from all architectural and service dependencies associated with other Fintech offerings. To test Vale. Adding a line here.

Testing

This is a **simple paragraph**. It uses passive voice.

In order to make things work, we just need to utilise the API.

This sentence could be clearer.
